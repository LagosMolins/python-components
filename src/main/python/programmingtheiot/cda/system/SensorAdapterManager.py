...
class SensorAdapterManager(object):
    ...
    def _initEnvironmentalSensorTasks(self):
        """
        Inicializa los sensores, ya sean simulados o emulados con Sense HAT.
        """

        if self.useEmulator:
            logging.info("Usando el emulador SenseHAT para sensores.")

            # Importar dinámicamente los módulos de los emuladores
            try:
                tempModule = import_module('programmingtheiot.cda.emulated.TemperatureSensorEmulatorTask')
                self.tempAdapter = getattr(tempModule, 'TemperatureSensorEmulatorTask')()

                humModule = import_module('programmingtheiot.cda.emulated.HumiditySensorEmulatorTask')
                self.humidityAdapter = getattr(humModule, 'HumiditySensorEmulatorTask')()

                presModule = import_module('programmingtheiot.cda.emulated.PressureSensorEmulatorTask')
                self.pressureAdapter = getattr(presModule, 'PressureSensorEmulatorTask')()

            except ImportError as e:
                logging.error(f"Error al cargar los módulos del emulador: {e}")
                raise RuntimeError("No se pudieron cargar los sensores emulados. Verifica que sense-emu esté instalado.")

        else:
            logging.info("Usando sensores simulados con SensorDataGenerator.")
    ...
    def startManager(self) -> bool:
        logging.info("Iniciando SensorAdapterManager.")

        if not self.scheduler.running:
            self.scheduler.start()
            return True
        else:
            logging.info("SensorAdapterManager ya está en ejecución.")
            return False

    def stopManager(self) -> bool:
        logging.info("Deteniendo SensorAdapterManager.")

        try:
            self.scheduler.shutdown()
            return True
        except:
            logging.info("El scheduler ya estaba detenido.")
            return False
