...
class MqttClientConnector(IPubSubClient):
	...
	def __init__(self, clientID: str = None):
		...
		if not clientID:
			...
			if not self.clientID:
				logging.warning("Client ID is None. Using default fallback ID.")
	...
	def onMessage(self, client, userdata, msg):
		...
		if payload:
			msgStr = str(payload.decode("utf-8"))
			logging.info('MQTT message received with payload: ' + msgStr)

			if self.dataMsgListener:
				self.dataMsgListener.handleIncomingMessage(msg.topic, msgStr)
		else:
